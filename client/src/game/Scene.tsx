...
const STARS: [number, number, number][] = [
  [3, 6, 2], [8, 14, 3], [12, 4, 2], [17, 9, 2], [21, 16, 3], [26, 3, 2], [30, 11, 2], [34, 7, 3],
  [39, 15, 2], [43, 5, 2], [47, 12, 3], [52, 8, 2], [56, 3, 2], [60, 14, 3], [64, 6, 2], [69, 10, 2],
  [73, 4, 3], [77, 13, 2], [81, 7, 2], [86, 16, 3], [90, 5, 2], [94, 11, 2], [97, 3, 3], [5, 22, 2],
  [15, 24, 2], [24, 20, 2], [37, 23, 3], [49, 19, 2], [58, 24, 2], [71, 21, 2], [83, 23, 3], [93, 20, 2],
  [10, 30, 2], [28, 28, 2], [45, 31, 2], [62, 29, 2], [79, 31, 2], [95, 28, 2], [19, 34, 2], [66, 35, 2],
];
...
export default function Scene({ hatchFlash = 0, bank = 0 }: { hatchFlash?: number; bank?: number }) {
  ...
  return (
    <div className="scene" aria-hidden="true">
      <div className="sky">
        {STARS.map(([x, y, s], i) => (
          <span key={i} className="star" style={{ left: `${x}%`, top: `${y}%`, ["--s" as string]: `${s}px` }} />
        ))}
      </div>

      <svg key={`lights-${bank}`} className={`lights ${bank > 0 ? "flash" : ""}`} viewBox="0 0 100 30" preserveAspectRatio="none">
        <path className="wire" d={`M0,${wireY(0, 7, 3)} Q50,${wireY(50, 7, 3) + 7} 100,${wireY(100, 7, 3)}`} vectorEffect="non-scaling-stroke" />
        <path className="wire" d={`M0,${wireY(0, 9, 12)} Q50,${wireY(50, 9, 12) + 9} 100,${wireY(100, 9, 12)}`} vectorEffect="non-scaling-stroke" />
        {BULBS.map((x) => (
          <ellipse key={`a${x}`} className="bulb" cx={x} cy={wireY(x, 7, 3) + 2.2} rx={0.55} ry={1.4} />
        ))}
        {BULBS.map((x) => (
          <ellipse key={`b${x + 2}`} className="bulb" cx={x + 2} cy={wireY(x + 2, 9, 12) + 2.2} rx={0.55} ry={1.4} />
        ))}
      </svg>
